...
zero runtime dependencies): it **additionally** generates a fresh random
...
`--pq`: an identity is encrypted as opaque bytes, so a hybrid keypair needs no
special handling. The wrapping key, not the identity string itself, is what
gets split: an age identity string is not uniformly random (a fixed prefix,
a restricted character set, a derived checksum), which the underlying
library's own README says to encrypt around rather than split directly. That
also means the underlying library's own documented gap — it does not itself
verify a reconstruction, so corrupt or mismatched shares can silently produce
wrong bytes — is closed by a real cryptographic check, not a heuristic:
AES-GCM's authentication tag rejects decryption outright under any
reconstructed key other than the exact one the identity was sealed with.
`sss-combine` additionally re-derives the recipient from the decrypted
identity and refuses if it does not match the recipient recorded (in
plaintext — recipients are public) on every share, which catches a forged
header on an otherwise-genuine, correctly-decrypting share set.
...
### The MCP caller is untrusted, and `snapshot_now` is fail-closed about it (#800)

Everything above is about an attacker who reaches the *machine*. The MCP server has a
second, narrower one built into its own contract: the caller is an AI agent that may be
steered by adversarial input it read somewhere, so every tool treats its arguments as
hostile. Most tools' caller-supplied paths are *write* destinations, and each is already
scoped (`wallet_create`'s `out` and `snapshot_now`'s `locator_file` to
`CYPHER_BRAIN_HOME`, `restore_now`'s `out_dir` warned and symlink-refused).

`snapshot_now` was the exception, and it ran the other way: `dirs` names the **plaintext**
and `recipients` names the **key**, so a caller that picked both could encrypt any
directory the server process can read to a key of its own choosing — over the free `file`
backend, which needs no `confirm_paid` and so crossed no consent gate at all. Two
operator-set environment variables now close that, and **both are enforced before any
work happens** — before the idempotency lookup, before the output file is created, before
the secret scan, the snapshot and the upload — so a refused call leaves no artifact, no
stored object and no idempotency record:

- **`CYPHER_BRAIN_PIN_RECIPIENTS` must resolve to at least one `age1…` key**, or *every*
  `snapshot_now` call is refused — and every recipient the call names must be on that
  allowlist. That takes the key half away from the caller: a caller-chosen `recipients`
  can no longer name an attacker's own public key. `snapshot` enforces the same rule
  (caveat 2 above) and remains the authoritative check; it is repeated at the MCP layer
  because an `idempotency_key` replay returns a stored result without ever reaching it,
  which would otherwise let a result recorded under an old pin be replayed under a new
  one.
- **`CYPHER_BRAIN_MCP_SOURCE_ROOTS`** (a JSON array of absolute paths) declares which
...
default that lets a call through, deliberately: `CYPHER_BRAIN_PIN_RECIPIENTS` is required
for **every** `snapshot_now` call, while `CYPHER_BRAIN_MCP_SOURCE_ROOTS` is required only
for a call that names `dirs` (a `pg`-only call needs no roots). How much of an existing
setup that breaks depends on what was already configured: if `CYPHER_BRAIN_PIN_RECIPIENTS`
already resolves to at least one `age1…` key (`snapshot` itself has used it for longer than
this policy has existed), a `pg`-only call keeps working and only a call naming `dirs`
newly needs `CYPHER_BRAIN_MCP_SOURCE_ROOTS`; if it does not resolve to a valid key at all
(unset, empty, or malformed), every call is refused. Either
way, whichever calls now fail get `ERR_POLICY_DENIED` / `CB-E025` until an operator sets
whichever of the two the refusal names and restarts it. That is the breaking part of this
change, and the only one; the CLI, the
...
retry its way past it, because neither input comes from the call.

What this does **not** do: it is not a sandbox, and it does not make an untrusted caller
safe in general. It scopes *which* directories that caller can name and *who* can decrypt
what it produces. Like every other path check on this server it is path-based, so a
directory swapped for a symlink after the check is followed by the snapshot all the same;
closing that needs descriptor-bound reads Node does not expose. And **the CLI is
deliberately unchanged** — a human at a shell already chooses what to encrypt, so neither
variable is required there. See MANAGEMENT.md's "MCP snapshot policy" for how to
configure both.
...
cypher-brain — encrypt a gbrain snapshot so only you can read it
  ...
  cypher-brain publish-latest --domain <name>.ton --from-locator-file <path> [--yes] [--wait <seconds>]
      ...
      printed alongside a tonviewer.com link (CYPHER_BRAIN_TON_TONVIEWER_URL, default
      https://tonviewer.com — override to https://testnet.tonviewer.com, alongside
      CYPHER_BRAIN_TON_TONAPI_URL, when running against testnet) and a warning: confirm
      the recipient Tonkeeper shows equals the printed address before approving anything.
...
Cumulative spend (single machine): CYPHER_BRAIN_MAX_SPEND_DAILY / CYPHER_BRAIN_MAX_SPEND_MONTHLY cap shared arweave/turbo receipts (winston/winc); CYPHER_BRAIN_TON_PROVIDER_MAX_SPEND_DAILY / CYPHER_BRAIN_TON_PROVIDER_MAX_SPEND_MONTHLY separately cap ton-provider receipts (nanoTON). UTC calendar day/month; 0/unset disables each cap. Only backends with a real cost model are counted — file and rclone pushes have no admission check at all and a well-formed pair of caps never counts against, or is charged by, either backend. Each pair must be internally consistent regardless of backend: an enabled MONTHLY cap smaller than its enabled DAILY cap is refused outright at startup for every command other than a --help/-h/help request (bare, or on any subcommand) or a bare --version/-V request (file/rclone included, same as any other malformed CYPHER_BRAIN_MAX_SPEND* value), since a UTC month always contains a full UTC day and the daily cap could then never be the actual binding constraint. Additive to the per-push caps, which must be positive when cumulative caps are enabled: each upload reserves the remaining per-push cap before put(), then reconciles to its durable receipt. Open/uncertain reservations remain charged until resolved, even across calendar boundaries. All callers must share one local receipt ledger; no multi-machine coordination. See MANAGEMENT.md for recovery.
