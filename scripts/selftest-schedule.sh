...
# never passes --recipient, so it snapshots to the single default identity and records
# the single-recipient warning EVERY run: pin N=1 (not just "some digit") so a broken
# counter (e.g. always 0, or double-counting the first run's warning into this one)
# would actually fail this check.
tail -n 1 "$LOG" | grep -qE '^OK rc=0 warnings=1$' || { echo "[FAIL] log does not end with OK rc=0 warnings=1 after the second run"; tail -n 3 "$LOG"; exit 1; }
...
tail -n 1 "$LOG" | grep -qE '^OK rc=0 warnings=1$' || { echo "[FAIL] log does not end with OK rc=0 warnings=1 after the 3rd (changed-content) run"; tail -n 3 "$LOG"; exit 1; }
...
if (j.last_run.rc_line !== 'OK rc=0 warnings=1') throw new Error('expected last_run.rc_line OK rc=0 warnings=1 (#432, single default identity), got ' + j.last_run.rc_line);
if (j.last_run.warning_count !== 1) throw new Error('expected last_run.warning_count 1 (#432), got ' + JSON.stringify(j.last_run.warning_count));
...
# Same single-default-identity reasoning as (c)/(c1b) above: pin N=1.
tail -n 1 "$LOG" | grep -qE '^OK rc=0 warnings=1$' || { echo "[FAIL] log does not end with OK rc=0 warnings=1 after the nested-index-dir run"; tail -n 3 "$LOG"; exit 1; }
...
if [ "$OS" != "Darwin" ]; then
  ...
  grep -qF "note: launchd plist $DRIFT_PLIST was already missing" "$TMP/drift-uninstall-drifted.log" \
  ...
  echo "[PASS] uninstall distinguishes plist drift (explicit 'note:' line, #529) from an ordinary removal ('removed:' line unchanged); exits 0 either way"
fi
