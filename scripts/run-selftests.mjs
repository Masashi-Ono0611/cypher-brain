...
const die = (msg) => {
  ...
  console.error('usage: node scripts/run-selftests.mjs [--jobs N]   (N >= 1; --jobs 1 = serial, for diagnosis)');
  process.exit(2);
};
...
// ── plan loading ───────────────────────────────────────────────────────────────
// CB_RUNNER_PLAN replaces the plan with one read from a JSON file. It exists for
// scripts/selftest-runner.sh, which has to make this runner fail on purpose — a pool that
// has never been watched stop scheduling, reap its children, or die on a signal is an
// unproven guard. It is not a supported way to run the suite: nothing in package.json or
// ci.yml sets it, and with it set the coverage guard checks the package.json the PLAN
// names, not this repo's.
const planPath = process.env.CB_RUNNER_PLAN;
...
if (planPath) {
  ...
  if (raw.packageJson) packageJsonPath = raw.packageJson;
  else packageJsonPath = null; // no package.json to check against
}
...
function checkCoverage() {
  ...
  const declared = Object.keys(scripts).filter((s) => s === 'selftest' || s.startsWith('selftest:'));
  const planned = new Set([...plan.parallel, ...plan.exclusive].map((t) => t.name));
  ...
  const stale = [...planned].filter((n) => (n.startsWith('selftest') || n === 'selftest') && !(n in scripts));
...
const killGroup = (child, sig) => {
  try {
    // Negative pid = the whole process group. `detached` below is what makes the child a
    // group leader, so this reaches the npm -> sh -> node chain and any daemon a test
    // backgrounded (arlocal, the mock TON seeder) rather than only the direct child.
