...
async function main(): Promise<void> {
  ...
  const hasHelpFlag = rest.some((tok, i) => (tok === '--help' || tok === '-h') && !helpScanSkip.has(i));
  ...
  // the command with the operator's settings silently absent. Deliberately UNCONDITIONAL
  // (unlike the spend-cap checks below) — an unreadable/unknown-key config file means
  // this run cannot trust ANY CYPHER_BRAIN_* value, including ones --version/--help
  // themselves might rely on going forward, so it is refused before even those (matches
  // scripts/selftest-config-file.sh, which uses `--version` as its refusal probe).
  if (CONFIG_FILE_ERROR) throw CONFIG_FILE_ERROR;
  // #928: computed BEFORE the spend-cap validation below, so a malformed
  // CYPHER_BRAIN_MAX_SPEND* value can never block --help/--version themselves — an
  // operator who fat-fingered a value needs --help to find out how to fix it, not a
  // wall of "error: ... must be an integer" on every subcommand including this one.
  // Covers both the bare top-level forms (`cypher-brain --help`/`-h`/`help`/
  // `--version`/`-V`, dispatched later by dispatchCommand()'s switch) and
  // `<command> --help` (handled immediately below, before dispatch). Unlike
  // CONFIG_FILE_ERROR just above, a malformed spend cap says nothing about whether
  // the config FILE loaded correctly, so gating only these checks is safe.
  const isHelpOrVersionRequest =
    hasHelpFlag || cmd === 'help' || cmd === '--help' || cmd === '-h' || cmd === '--version' || cmd === '-V';
  if (!isHelpOrVersionRequest) {
    // #715/#928: same posture as CONFIG_FILE_ERROR above — CYPHER_BRAIN_MAX_SPEND/
    // CYPHER_BRAIN_TON_PROVIDER_MAX_SPEND used to call BigInt() directly at
    // module-import time, so a non-integer value (e.g. "0.5") threw a raw SyntaxError
    // before this function ever ran, crashing every command. config.ts now records the
    // failure instead; re-enter the normal error path here — but only for a command
    // that would actually READ one of these values. A help/version request never does
    // (see isHelpOrVersionRequest above), so these checks are gated on it: #928 found
    // that even after the #715 fix moved the throw out of module-import time, it still
    // ran ahead of the --help early-return just below, leaving `cypher-brain --help`/
    // `cypher-brain push --help` themselves blocked by an unrelated malformed env var.
    ...
    // #928: MONTHLY < DAILY within a pair — see checkSpendCapOrder's comment in config.ts.
    if (AR_MAX_SPEND_ORDER_ERROR) throw AR_MAX_SPEND_ORDER_ERROR;
    if (TON_PROVIDER_MAX_SPEND_ORDER_ERROR) throw TON_PROVIDER_MAX_SPEND_ORDER_ERROR;
  }
  if (hasHelpFlag) {
