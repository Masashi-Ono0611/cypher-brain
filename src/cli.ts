...
const HELP = `cypher-brain — encrypt a gbrain snapshot so only you can read it
  ...
  cypher-brain wallet balance [--wallet <path>] [--address <addr>] [--json] [--chain arweave|ton]
      ...
      with --address) plus an approximate USD line (tonUsdRate) — the same fields
      --json emits as one JSON line ({address, balance_nanoton, status}).
  ...
  cypher-brain push --in <file.age> --backend <file|arweave|turbo|rclone|ton|ton-provider> [--remote <name>:<path>] [--yes] [--save-locator <path>] [--skip-unchanged] [--digest <hex>] [--force]
      ...
      instead. (If BOTH are set and disagree, the wallet's own address wins —
      auto-signing requires sender==owner on-chain, so TON_PROVIDER_OWNER is ignored
      with a warning rather than silently producing a deploy the wallet can't
      authorize.) Before signing/printing the deeplink it runs an advisory pre-deploy
