...
`CYPHER_BRAIN_TON_PROVIDER_OWNER`/`CYPHER_BRAIN_TON_PROVIDER_MAX_SPEND`/a built
notify binary) still exits cleanly with setup guidance instead of failing deep inside
`push()` — nothing is rolled back, so the identity/keys this run already set up before
reaching the backend prompt are preserved exactly as they were, the same way
arweave/turbo's wallet check already worked.
...
advisory pre-deploy funds check against the owner address's on-chain balance, through
the same `warn()` chokepoint `turbo`'s own funds check uses (WARN only, never a hard
abort — the deploy is always signed by a human in their own wallet app, so an actual
shortfall gets its own unambiguous refusal there regardless); shares
