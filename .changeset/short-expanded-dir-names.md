...
and readable — `expanded/001-memory-<64-hex-char digest>/` instead of a
...
never land in the same directory (#181); the full, un-truncated SHA-256
digest of the full source path is now appended too, so two DIFFERENT sources
sharing a basename won't collide even across two SEPARATE restores into the
same `--out-dir`, where nothing ties their manifests' component order
together (`source` is attacker-controlled data, so this deliberately uses
the full digest rather than a shortened one — see `sourceDigest()`'s doc
comment in `src/lib/restore.ts` for the review history behind that choice).
