...
see next paragraph — and refuses every command other than a `--help`/`-h`/
`help` request (bare, or on any subcommand) or a bare `--version`/`-V`
request, `file`/`rclone` pushes included, the same way an unparseable
`CYPHER_BRAIN_MAX_SPEND_DAILY` already did before this admission check
existed.)
...
`--help`/`-h`/`help` request (bare, or on any subcommand) or a bare
`--version`/`-V` request, rather than silently only ever enforcing the
monthly figure.
...
> (a `pg`-only call needs no roots). How much breaks depends on what was already
> configured: if `CYPHER_BRAIN_PIN_RECIPIENTS` already resolves to at least one `age1…`
> key (`snapshot` itself has used it for longer than this policy has existed), your
> `pg`-only calls keep working and only a call naming `dirs` newly needs
> `CYPHER_BRAIN_MCP_SOURCE_ROOTS`; if it does not resolve to a valid key at all (unset,
> empty, or malformed), every call is refused. Either way, whichever calls now fail get
> `ERR_POLICY_DENIED` / `CB-E025` until you set whichever of the two the refusal names
> and restart it. Nothing else — the CLI, the nightly schedule, every other MCP tool —
> is affected.
...
| CB-E006 | One of three independent causes, named explicitly in the refusal message itself: (a) the upload's cost estimate (or a per-run signed push's combined ciphertext+`.minisig` total) exceeds `CYPHER_BRAIN_MAX_SPEND`; (b) the combined total exceeds one of the daily/monthly cumulative caps from "Daily and monthly spend admission" above (`CYPHER_BRAIN_MAX_SPEND_DAILY`/`_MONTHLY`, `CYPHER_BRAIN_TON_PROVIDER_MAX_SPEND_DAILY`/`_MONTHLY` — the message also states the exact UTC receipts/open-reservations total); or (c) the signing wallet's balance/credits are insufficient. | For (a): raise (or intentionally lower) `CYPHER_BRAIN_MAX_SPEND`. For (b): raising the per-push cap does NOT help — either wait for the UTC day/month window to roll over, raise the relevant `_DAILY`/`_MONTHLY` variable, or reconcile the open reservations named in the message (see "Daily and monthly spend admission" above). For (c): fund the wallet (Turbo Credits top up at app.ardrive.io; Arweave L1 needs AR in the JWK's own address). |
| CB-E007 | `push` targets a paid backend (`arweave`/`turbo`/`ton-provider`) without consent — `arweave`/`turbo` are permanent stores, `ton-provider` is availability-while-the-provider-renews, not permanent (see `docs/durability.md`). (#781: narrowed from also matching CB-E024's "no spend cap configured" refusals below — `--yes` cannot fix those.) | Pass `--yes`, or set `CYPHER_BRAIN_YES=1` (the unattended-cadence escape hatch) to confirm the spend. |
...
| CB-E009 | The command's output path already exists and no-clobber refused to overwrite it (protects a prior snapshot/pull result from silent loss) — or `push --backend rclone --remote <name>:<path>` found an object already sitting at that exact remote path (#533: rclone destinations are operator-named, not content-addressed, so a reused `--remote` could otherwise silently replace a different snapshot). Never `--save-locator`: that file is intentionally and unconditionally replaced (an atomic write-temp-then-rename over whatever was there) and never produces this. | Pick a different `--out`/`--remote` path, move the existing file aside, or pass `--force` to overwrite it deliberately. |
...
| CB-E014 | The MCP `schedule_status` tool (and `doctor`'s internal `scheduleStatusReport()` read) ran before `schedule install` — the CLI's own `schedule status`/`uninstall` deliberately do NOT error for this: they treat "nothing installed" as a normal, expected result (exit 0). Or: writing the crontab entry failed, from `install`, `uninstall`, or the legacy-entry migration. | Run `cypher-brain schedule install` first; a crontab-write failure usually means missing cron permissions/availability in this environment. |
...
| CB-E016 | `restore`/`verify` found a `*.minisig` signature next to the ciphertext, and a configured signing public key, but the signature did NOT verify — the artifact may be tampered or forged. Also fires from `witness verify`'s chain authentication: a witness-catalog entry's signature that does not verify against the trusted key (a mismatched `--sig-locator`, or `--pubkey` pointed at the wrong signer) — the witness catalog entry may be tampered or forged (#933; exits 3, not 1 — see `witness --help`). | Do not trust the artifact. Confirm you're checking against the correct `sign-recipient.pub` (or `--sign-recipient`); if it matches and the failure persists, treat the ciphertext as compromised and re-fetch from a trusted copy. For the witness scenario, confirm `--pubkey`/`--sig-locator` are the correct pairing for this locator; if they are, treat the witness chain as compromised. |
...
| CB-E025 | The MCP server refused a `snapshot_now` call under its fail-closed snapshot policy (#800): either `CYPHER_BRAIN_PIN_RECIPIENTS` does not resolve to at least one `age1…` key, or the call named `dirs` and `CYPHER_BRAIN_MCP_SOURCE_ROOTS` is unset/empty/malformed or does not cover one of those directories. MCP only — the CLI `snapshot` has no such gate. A caller cannot fix this by retrying with different arguments: both inputs are operator environment settings. | Set whichever of the two variables the refusal message names (not necessarily both — a `pg`-only call needs no roots) in the MCP server's own environment and restart it — see "MCP snapshot policy" above. |
| CB-E026 | `restore` or `verify` found that `--in` was **rewritten in place while it was being read**: two of the run's own passes over the same file hashed differently, so the `[PASS]` lines above describe more than one set of bytes. Nothing was promoted — on `restore`, `--out-dir` was not touched, no `pg_restore` ran, and the scratch tree was removed; on `verify`, no verdict was printed. | Find out what is writing to `--in` mid-run (a concurrent `pull`, a sync client, another process sharing the path), then run against a path only you can write — ideally with `--sha256` and `--require-signature` so a substituted artifact is refused before any of this rather than after. |
| CB-E027 | A paid push ended with the outcome **UNCERTAIN**: the payment may already have happened and nothing this process can read says whether it did. Either an `arweave` L1 POST that never answered (or was refused) whose follow-up probe of the signed tx id found nothing, or a `ton-provider` deploy whose broadcast failed *after* the signed message had already left this process and whose contract-address probe stayed inconclusive. Not finding it is **not** proof it is absent — a just-posted transaction is not immediately indexed. | Verify on-chain **before** retrying, using the identifier the message names (the Arweave tx id at `https://arweave.net/tx/<id>/status`, or the TON contract address on an explorer). If it landed, do not push again — record the locator/address. If it did not, retry with a **new** idempotency key: over MCP the old key is permanently blocked on purpose (see "MCP idempotency keys" below). A `ton-provider` deploy that ends this way also leaves a `pending` entry in `$CYPHER_BRAIN_HOME/receipt-ledger.jsonl.pending-spends.jsonl` (#808), which `cypher-brain doctor`'s `pending-spend-intents` check keeps reporting until it is settled — so the reminder to go and look survives the session that produced it. |
| CB-E028 | Another push is **already running** against the same `--save-locator` file (#806), the same rclone `--remote` (#807), or the same derived `ton-provider` StorageV1 contract address (#948), and was still running after this one waited a few seconds for it. Nothing was uploaded and nothing was paid: the races this prevents are two pushes both paying for the same content while only one locator survives in the file, two pushes both writing one rclone remote so the later silently destroys the earlier snapshot, and two pushes both broadcasting a funding transfer to the same TON contract (a real double payment — see `ton-provider`'s own already-active check, which this lock wraps end-to-end from that check through on-chain confirmation, precisely because two genuinely concurrent processes can both observe "not yet funded" before either one's transfer lands). A lock left behind by a **crashed** push never causes this — the next run sees that its owner is gone (its pid is not running), says so, clears it and proceeds — and a lock more than a day old is cleared regardless, so even a pid that has since been reused by an unrelated live process cannot wedge the schedule permanently, so an unattended `schedule` cannot be wedged by one. Note the converse: clearing such a lock does **not** prove the crashed run had not already paid — check `receipt-ledger.jsonl` if it died mid-push. The lock key resolves a `--save-locator` path through symlinks (including the file itself, when it already exists) so two spellings of one file collapse onto the same lock — but two names that are the SAME file only through a hard link, or two spellings that differ only in case on a case-insensitive filesystem, still produce independent locks and are not caught by this. | Wait for the other push to finish and re-run; `--force` is not the answer and does not bypass this. The message names the lock file (and, for the `ton-provider` case, the contract address). Remove it by hand only in the one case that is not self-healing: a still-running process that leaked the lock instead of releasing it (which prints its own warning when it happens). |

`restore`/`verify` open `--in` once and run every phase — the `--sha256` pin, the
`*.minisig` check, the size cap, the tar-entry inspection and the extraction — off that
one file descriptor (#785). Renaming a different artifact onto `--in` mid-run therefore
changes nothing about what gets restored; rewriting the same file in place is what
CB-E026 above reports, and it is caught by requiring every pass that read the file to
end up with the same sha256, not just the last one. `verify --level drill` hands its
descriptor to the restore step rather than reopening the path, so the signature it
verified and the bytes it drills are necessarily the same. Neither behaviour is
configurable, and the only visible cost is one extra read pass over `--in` (the baseline
hash) on every restore.
...
| Restore input binding (`restore`/`verify` open `--in` once and run every phase off that descriptor, so a rename onto the path mid-run cannot substitute what gets extracted; an in-place rewrite of the same file is caught by requiring every pass that read it to agree on one sha256, and refused as CB-E026 before anything is promoted — #785) | **proven** — `selftest:restore-toctou` (CI; the swap is injected at a deterministic pre-extraction barrier, both attack shapes are confirmed to have landed, a `*.minisig` made over other bytes is shown not to authenticate these, and the descriptor owner's own guarantees — cross-phase digest recording, rename immunity, the streamed size cap — are exercised directly) |
