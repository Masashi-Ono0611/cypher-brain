...
| CB-E007 | `push` targets a paid, permanent backend (`arweave`/`turbo`/`ton-provider`) without consent. (#781: narrowed from also matching CB-E024's "no spend cap configured" refusals below — `--yes` cannot fix those.) | Pass `--yes`, or set `CYPHER_BRAIN_YES=1` (the unattended-cadence escape hatch) to confirm the spend. |
